...
<div class="wrap-app">	
	<div class="row app-body">
     ...
     <div class="row">
         ...
         <div class="small-4 columns">
          <input type="button" class="button-submit" value="Connect"   />
         </div>
   ...
   <c:choose>
    <c:when test="${requestScope.supplierDetails!=null}">   
				<div class="row">
			...
			<table class="table-transactions">
				...
				<c:forEach var="listItems" items="${requestScope.supplierDetails}">
					<tr>
						<td><c:out value='${listItems.supplierName}'/>  </td>
						<td>    
			 	 
						<%--  <input type="button" class="button-add" value="Approve" onClick="javascript:approvehandShaking(<c:out value='${listItems.supplierId}'/>);">  --%> 
     <c:choose>
    <c:when test="${ !listItems.isActive}">
   
  <a class="button-add" href="javascript:approvehandShaking(<c:out value='${listItems.supplierId}'/>);">Approve</a>
    </c:when>
    <c:otherwise>
       Active
...
<td> <i class="foundicon-trash" onClick="javascript:deleteHandShaking(<c:out value='${listItems.id}'/>);"></i> </td> 
    ...
    <c:otherwise>
     <div class="large-8 columns" id="Addbutton">No Connection Found! <input type="button"  class="button-add" value="SEND REQUEST" onclick="javascript:showHideDiv('1');"/> </div>
    </c:otherwise>
    </c:choose> 		
